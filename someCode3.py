...
# Load data
train = pd.read_csv('cleaned_training.csv')
...
train = df[df['DiagPeriodL90D'] != 2].copy()
test = df[df['DiagPeriodL90D'] == 2].copy()

# Drop 'DiagPeriodL90D' from test data
#test.drop(columns=['DiagPeriodL90D'], inplace=True)

# Drop rows with NaN values in the target variable
train = train.dropna(subset=['DiagPeriodL90D'])
...
# Write patient IDs from test data as the index
submission.index = test.index

# Save the submission file
submission.to_csv('submission_StratifiedKFold.csv', index=True)
